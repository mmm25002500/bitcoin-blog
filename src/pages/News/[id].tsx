...
export const getStaticProps: GetStaticProps = async (context) => {
  ...
  try {
    ...
    const response = await fetch(
      `${baseUrl}/api/getArticleMarkdown?id=${encodeURIComponent(String(id))}&type=News`,
    );
    ...
    // 用編號或舊網址進來，而新聞有自訂網址（或網址改過）→ 轉到目前的網址
    if (matchedBy === "old_slug" || (matchedBy === "id" && data.slug)) {
      return {
        redirect: { destination: articleHref("News", data), permanent: false },
        revalidate: 60,
      };
    }
