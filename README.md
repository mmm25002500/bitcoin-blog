...
| API | Input | Output | 簡介 |
| --- | --- | --- | --- |
| getArticalLinkByFilename | `req.query: { filename: string }` | `res: { title: string, authorData: { fullname?: string, name?: string, img?: string, description?: string, id: string }, date: string, description: string, link: string }` | 根據文件名獲取文章的相關鏈接和信息 |
